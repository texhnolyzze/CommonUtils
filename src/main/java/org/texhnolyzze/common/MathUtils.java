...
public final class MathUtils {
    ...
    /**
     * Возвращает величину пересечения двух отрезков [a1, b1] - [a2, b2]
     * Если пересечения нет -- возвращает -1
     */
    public static long overlapping(long a1, long b1, long a2, long b2) {
        final long maxa = Math.max(a1, a2);
        final long minb = Math.min(b1, b2);
        if (maxa <= minb) {
            return minb - maxa;
        }
        return -1;
    ...
    /**
     * Возвращает начальную точку пересечения двух отрезков [a1, b1] - [a2, b2]
     * Если пересечения нет -- возвращает Long.MAX_VALUE
     */
    public static long overlappingStart(long a1, long b1, long a2, long b2) {
        final long maxa = Math.max(a1, a2);
        final long minb = Math.min(b1, b2);
        if (maxa < minb) {
            return maxa;
    ...
    public static class WGS84 {
        ...
        public static double getDistanceMeters(double lat1, double lon1, double lat2, double lon2) {
            ...
            double phi1 = toRadians(lon1);
            double phi2 = toRadians(lon2);
        ...
        public static void packSphereRectangleWithSphereRectangles(
            double lat1,
            double lon1,
            double lat2,
            double lon2,
        ...
        private static vec3 p(double teta, double phi, vec3 dest) {
            return dest.set(
                EARTH_RADIUS_METERS * (cos(phi) * cos(teta)),
